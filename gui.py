...
import datetime
import os
from config import LOGO_DIR, MAX_MATCHES_IN_LIST, MAX_LEGS_IN_EXPRESS
try:
...
def create_main_gui(on_generate_callback):
    # ... (здесь весь код функции create_main_gui из нашего предыдущего обсуждения) ...
    # (я скопировал его, внеся исправления)
    root = tk.Tk(); root.title("Генератор постов v0.3"); root.geometry("700x850")
    ...
    single_fields_config = [("Название команды 1", "team1_name", "Combobox_Team"), ("Название команды 2", "team2_name", "Combobox_Team"), ("Текст VS", "vs_text", "Entry"), ("Коэффициент", "coefficient", "Entry"), ("Прогноз", "prediction", "Entry_Large"), ("Дата", "date_single", "Entry"), ("Фон", "background_path_single", "Entry_File")]
    for i, (label_text, key, widget_type) in enumerate(single_fields_config):
        ...
        if widget_type == "Combobox_Team":
            widget = ttk.Combobox(single_post_frame, width=35, values=team_names_for_combobox, state="normal"); widget.bind('<KeyRelease>', lambda ev, e=widget, o=team_names_for_combobox: update_combobox_filter(e,o))
    ...
    express_general_fields_config = [("Заголовок экспресса (необяз.)", "express_title", "Entry"), ("Общий коэффициент экспресса", "total_coefficient", "Entry"), ("Дата экспресса", "date_express", "Entry"), ("Фон для экспресса", "background_path_express", "Entry_File")]
    ...
    leg_fields_config = [("Команда 1 события", "leg_team1_name", "Combobox_Team"), ("Команда 2 события", "leg_team2_name", "Combobox_Team"), ("Ставка на событие", "leg_bet_text", "Entry"), ("Коэфф. этого события", "leg_coefficient", "Entry")]
    for i, (label_text, key, widget_type) in enumerate(leg_fields_config):
        ...
        if widget_type == "Combobox_Team":
            widget = ttk.Combobox(leg_frame, width=30, values=team_names_for_combobox, state="normal"); widget.bind('<KeyRelease>', lambda ev, e=widget, o=team_names_for_combobox: update_combobox_filter(e,o))
    ...
    legs_display_frame = ttk.LabelFrame(express_post_frame, text=f"События в текущем экспрессе (макс. {MAX_LEGS_IN_EXPRESS})", padding="10"); legs_display_frame.grid(row=len(express_general_fields_config)+1, column=0, columnspan=3, pady=10, sticky="ew")
    ...
    main_add_button = ttk.Button(action_buttons_frame, text="Добавить 'Одиночный прогноз' в список", command=lambda: add_current_post_to_list()); main_add_button.pack(side=tk.LEFT, padx=10, expand=True, fill="x")
    generate_all_button = ttk.Button(action_buttons_frame, text="Создать все изображения из списка", command=lambda: on_generate_callback(match_data_list, root)); generate_all_button.pack(side=tk.LEFT, padx=10, expand=True, fill="x")
    ...
    post_type_combo.bind("<<ComboboxSelected>>", on_post_type_change_ui); on_post_type_change_ui()
    ...
    def add_leg_to_ui_buffer():
        if len(current_express_legs_buffer) >= MAX_LEGS_IN_EXPRESS: messagebox.showwarning("Лимит", f"Максимум {MAX_LEGS_IN_EXPRESS} событий в экспрессе."); return
        ...
        except ValueError: messagebox.showerror("Ошибка", "Коэффициент события должен быть числом!"); return
        team1_logo_ref = TEAM_MAPPINGS.get(team1_name.lower(), team1_name); team2_logo_ref = TEAM_MAPPINGS.get(team2_name.lower(), team2_name)
    ...
    def add_current_post_to_list():
        ...
        if post_type == "Одиночный прогноз":
            ...
            if not total_coeff_str: messagebox.showerror("Ошибка", "Укажите общий коэффициент!"); return
            try: float(total_coeff_str.replace(',','.'))
            except ValueError: messagebox.showerror("Ошибка", "Общий коэффициент должен быть числом!"); return
