...
def create_main_gui(on_generate_callback):
    root = tk.Tk(); root.title("Генератор постов v0.4"); root.geometry("700x850")
    ...
    single_fields_config = [
        ("Название команды 1", "team1_name", "Combobox_Team"), ("Название команды 2", "team2_name", "Combobox_Team"),
    ...
    express_general_fields_config = [
        ("Заголовок экспресса (необяз.)", "express_title", "Entry"), ("Общий коэффициент экспресса", "total_coefficient", "Entry"),
    ...
    def on_post_type_change_ui(event=None):
        ...
        if selected_type == "Одиночный прогноз":
            single_post_frame.pack(fill="both", expand=True); express_post_frame.pack_forget(); main_add_button.config(text="Добавить 'Одиночный прогноз' в список")
        elif selected_type == "Экспресс":
            single_post_frame.pack_forget(); express_post_frame.pack(fill="both", expand=True); main_add_button.config(text="Добавить 'Экспресс' в список")
    ...
    def remove_selected_leg_from_ui_buffer():
        ...
        try:
            ...
            messagebox.showerror("Ошибка", "Не удалось удалить выбранный элемент.")

    def add_current_post_to_list():
        # ... (код add_current_post_to_list без изменений) ...
    ...
    # --- НОВЫЕ ФУНКЦИИ ДЛЯ ЗАПОЛНЕНИЯ ПРИМЕРОВ ---
    def fill_with_single_example():
        # Очищаем поля перед заполнением
        for key in ["team1_name", "team2_name", "coefficient", "prediction"]:
            if key in single_entries: single_entries[key].delete(0, tk.END)
        # Заполняем данными
        single_entries["team1_name"].set("Спартак")
        single_entries["team2_name"].set("Зенит")
        single_entries["coefficient"].insert(0, "2.55")
        single_entries["prediction"].insert(0, "Победа Спартака или ничья (1X)")
        
    def fill_with_express_example():
        # Очищаем общие поля и буфер
        ...
        express_general_entries["express_title"].insert(0, "Экспресс на выходные")
        express_general_entries["total_coefficient"].insert(0, "7.45")
        
        # Добавляем примеры событий в буфер
        example_legs = [
            ...
            {"team1_name": "Бавария", "team1_logo_ref": TEAM_MAPPINGS.get("бавария", "Бавария"), "team2_name": "Боруссия Д", "team2_logo_ref": TEAM_MAPPINGS.get("боруссия д", "Боруссия Д"), "bet_text": "П1", "leg_coefficient": "1.50"}
        ]
        current_express_legs_buffer.extend(example_legs[:MAX_LEGS_IN_EXPRESS]) # Добавляем, но не больше лимита
        
        # Обновляем таблицу
        update_legs_treeview()
