# gui.py (ВЕРСИЯ С РАСШИРЕННОЙ ДИАГНОСТИКОЙ)
...
def create_main_gui(on_generate_callback):
    root = tk.Tk(); root.title("Генератор постов v0.5 (Debug)"); root.geometry("700x850")
    ...
    single_entries["team1_name"].bind('<KeyRelease>', lambda ev, e=single_entries["team1_name"], o=team_names_for_combobox: update_combobox_filter(e,o))
    single_entries["team2_name"].bind('<KeyRelease>', lambda ev, e=single_entries["team2_name"], o=team_names_for_combobox: update_combobox_filter(e,o))
    express_leg_entries["leg_team1_name"].bind('<KeyRelease>', lambda ev, e=express_leg_entries["leg_team1_name"], o=team_names_for_combobox: update_combobox_filter(e,o))
    express_leg_entries["leg_team2_name"].bind('<KeyRelease>', lambda ev, e=express_leg_entries["leg_team2_name"], o=team_names_for_combobox: update_combobox_filter(e,o))
    ...
    # --- НОВАЯ ФУНКЦИЯ-ПОМОЩНИК ДЛЯ ПОИСКА ЛОГО ---
    def get_logo_ref_for_team(team_name_original):
        """
        Инкапсулирует логику поиска логотипа с подробной отладкой.
        Возвращает полный путь к файлу или оригинальное имя, если ничего не найдено.
        """
        print("-" * 20)
        print(f"Поиск логотипа для: '{team_name_original}'")
        
        team_name_lower = team_name_original.lower()
        print(f"1. Поисковый ключ (lower case): '{team_name_lower}'")

        logo_filename = TEAM_MAPPINGS.get(team_name_lower)
        
        if logo_filename:
            print(f"2. Успех! Найдено сопоставление в TEAM_MAPPINGS: '{team_name_lower}' -> '{logo_filename}'")
            full_logo_path = os.path.join(LOGO_DIR, logo_filename)
            print(f"3. Сформирован полный путь: '{full_logo_path}'")
            
            if os.path.exists(full_logo_path):
                print(f"4. ПРОВЕРКА ПРОШЛА: Файл по этому пути СУЩЕСТВУЕТ. Возвращаем путь.")
                return full_logo_path
            else:
                print(f"4. ПРОВЕРКА ПРОВАЛЕНА: Файл по пути '{full_logo_path}' НЕ СУЩЕСТВУЕТ.")
                messagebox.showwarning("Файл не найден", f"Файл логотипа '{logo_filename}' не найден в папке {LOGO_DIR}.")
                return team_name_original # Возвращаем имя как плейсхолдер
        else:
            print(f"2. ПРОВАЛ: Ключ '{team_name_lower}' НЕ НАЙДЕН в словаре TEAM_MAPPINGS.")
            messagebox.showwarning("Сопоставление не найдено", f"Для команды '{team_name_original}' не найдено сопоставление в team_mappings_fixed.py.")
            return team_name_original # Возвращаем имя как плейсхолдер
    ...
    def add_leg_to_ui_buffer():
        ...
        team1_logo_ref = get_logo_ref_for_team(team1_name)
        team2_logo_ref = get_logo_ref_for_team(team2_name)
