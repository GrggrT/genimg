...
def create_main_gui(on_generate_callback):
    ...
    for i, (label_text, key, widget_type) in enumerate(single_fields_config):
        ...
        if widget_type == "Combobox_Team":
            ...
            # bind будет определен позже, когда все функции будут созданы
    ...
    # --- Вспомогательные функции GUI (определяем их ДО вызова) ---
    ...
    # Привязываем обработчик события здесь, после того как функция определена
