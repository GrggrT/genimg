# image_generator.py
# Модуль, отвечающий за создание изображений и отрисовку всех элементов.
...
# --- Вспомогательные функции отрисовки (как и были, с небольшими улучшениями) ---
...
def draw_text_with_effects(draw, text_block, center_xy, font, color, shadow_color, stroke_color, is_multiline=False):
    ...
    # Для textbbox используем тот же spacing, что и для отрисовки
    if is_multiline:
...
def create_single_image(match_content, background_path, output_filename):
    # ... (весь код функции create_single_image остается как был) ...
    image = None;
    ...
    for idx, cell in enumerate(user_defined_cells):
        ...
        if cell['type'] == 'team_name': font_params = {'initial_font_size': 50, 'min_font_size': 18, 'max_lines': 3}; text_color = '#E0E0E0'
            ...
            except Exception as e: print(f"Ошибка лого/плейсхолдера '{logo_path}': {e}")
            continue
...
def create_express_image(express_content, background_path, output_filename):
    ...
    padding = 20 # Общие отступы по краям изображения
    ...
    if express_content.get("express_title"):
        ...
        header_y_pos += len(lines_title) * font_title.size + 10 # Сдвигаем вниз после заголовка
    ...
    table_bottom_y = IMAGE_SIZE[1] - 200 # Оставляем место для подвала
    ...
    if num_legs == 0:
        # Если событий нет, можно нарисовать сообщение
        ...
        # Задаем ширину колонок в процентах
        ...
        for i, leg in enumerate(legs):
            ...
            # --- Колонка 1: Матч (Команды и логотипы) ---
            col_match_width = table_width * col_widths_percent['match']
            col_match_x_start = padding
            
            # Логотипы
            logo_h = int(row_height * 0.6) # Высота лого = 60% от высоты строки
            logo_w = logo_h
            ...
            try:
                ...
                if os.path.exists(leg["team1_logo_ref"]):
                    logo1 = Image.open(leg["team1_logo_ref"]).convert("RGBA")
                    logo1.thumbnail((logo_w, logo_h), Image.Resampling.LANCZOS)
                    image.paste(logo1, (int(col_match_x_start + padding), logo_y_pos), logo1)
                # Лого 2
                if os.path.exists(leg["team2_logo_ref"]):
                    logo2 = Image.open(leg["team2_logo_ref"]).convert("RGBA")
                    logo2.thumbnail((logo_w, logo_h), Image.Resampling.LANCZOS)
                    image.paste(logo2, (int(col_match_x_start + col_match_width - logo_w - padding), logo_y_pos), logo2)
            except Exception as e: print(f"Ошибка отрисовки лого в экспрессе: {e}")

            # Текст команд
            teams_text = f"{leg['team1_name']} - {leg['team2_name']}"
            teams_text_width_target = col_match_width - 2*logo_w - 2*padding
