...
def get_font_for_cell(draw_context, text_to_fit, target_width, target_height, initial_font_size=40, min_font_size=10, max_lines=3, line_spacing_ratio=1.2):
    # ... (вставьте сюда полный код функции get_font_for_cell из предыдущего нашего ответа)
    # Это надежная версия, которая проверяет ширину и высоту.
    ...
    while current_font_size >= min_font_size:
        ...
        block_bbox = draw_context.multiline_textbbox((0,0), "\n".join(lines), font=font, spacing=int(current_font_size*0.2))
...
def draw_text_with_effects(draw, text_block, center_xy, font, color, shadow_color, stroke_color, is_multiline=False):
    # ... (вставьте сюда полный код функции draw_text_with_effects)
    x, y = center_xy
...
def create_gradient_bg(width, height, color1, color2):
    # ... (вставьте сюда полный код функции create_gradient_bg)
...
def create_single_image(match_content, background_path, output_filename):
    # ... (вставьте сюда полный код функции create_single_image, использующий user_defined_cells)
    image = None
    ...
    try: image.save(output_filename, quality=IMAGE_QUALITY); print(f"Создано: {output_filename}")
...
def create_express_image(express_content, background_path, output_filename):
    ...
    temp_img = Image.new("RGB", IMAGE_SIZE, "darkblue")
